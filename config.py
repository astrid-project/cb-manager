...
class Config:
    ...
    def accepted(self, func):
        return {
            'delete': self.namespace.response(status.HTTP_202_ACCEPTED, f'{self.target.LABEL}s currectly deleted', Document.response_model),
            'put': self.namespace.response(status.HTTP_202_ACCEPTED, f'{self.target.LABEL} with the given ID currectly updated', Document.response_model)
