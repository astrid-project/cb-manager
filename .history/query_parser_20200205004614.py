...
class QueryParser:
    ...
    def _where(self, query):
            ...
            for op, clause in query.get('where', {}).items():
                if op == 'and':
                    ...
                    prop = self._fix_target(clause.get('target', None))
                    ...
                    if prop is not None and expr is not None:
                        ...
                        raise falcon.HTTPBadRequest(title='Request not valid', description=f'{op} clause with not valid/missing data')
    ...
    def _order(self, query):
        ...
        for order in query.get('order', []):
            prop = self._fix_target(order.get('target', None))
