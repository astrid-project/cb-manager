    ...
    def on_post(self, req, resp, id=None):
        ...
        if id is not None:
            if type(query) is list:
                raise falcon.HTTPBadRequest(title='id provided', description=f'Request can create only 1 new {self.doc_name}')
            single = True
        else:
            single = False
        for data in wrap(query):
            ...
            if data_id is None and not single:
                ...
                raise falcon.HTTPBadRequest(
                    title='id provided', description='The request body must not contain the id')
            try:
